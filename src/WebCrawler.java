...
public class WebCrawler 
{
	...
	public static HashMap<Integer, String> topAnimes() throws IOException
	{
		...
		for(Element e : body.select("tr")) 
		{
			number++;
			//================================Title================================
			String title = e.select("td.title.al.va-t.word-break img").attr("alt")
					.replaceAll("&#039;", "'").replaceAll("Anime: ", "");
		...
		map.entrySet()
		  .stream()
		  .sorted(HashMap.Entry.<Integer, String>comparingByKey());
	...
	public static ArrayList<SearchDetails> searchAnime(String searchStr) throws IOException
	{
		...
		try
		{
			...
			Connection.Response response = Jsoup.connect(url + searchStr)
			...
			//Loop through the elements "div.js-categories-seasonal.js-block-list.list" and selects "tr" element
			...
			for(Element e : body.select("tr").subList(1, Math.min(16, body.select("tr").size()))) 
				{
					//================================Title================================
					String title = e.select("a.hoverinfo_trigger img").attr("alt")
							.replaceAll("&#039;", "'");
	...
	{	
		...
		//Loop through the elements "div.js-scrollfix-bottom-rel" and selects "tr" element
		...
		for(Element e : body2.select("tr"))
		{
			...
			//================================Synopsis================================
			String description = e.select("p[itemprop = description]").text()
